...
if [ "${GENS}" = "all" ] || grep -qw "client" <<<"${GENS}"; then
  ...
  "${gobin}/client-gen" --plural-exceptions="RSS:RSSes" --clientset-name "${CLIENTSET_NAME_VERSIONED:-versioned}" --input-base "" --input "$(codegen::join , "${FQ_APIS[@]}")" --output-package "${OUTPUT_PKG}/${CLIENTSET_PKG_NAME:-clientset}" "$@"
fi
